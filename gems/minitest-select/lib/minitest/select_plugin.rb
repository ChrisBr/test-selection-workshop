...
require_relative "select/min_length_filter"
...
module Minitest
  class << self
    def plugin_select_options(opts, options)
      ...
      opts.on "--select-min-length=[LENGTH]", "Run simple reporter" do |length|
        options[:select_min_length] = length.to_i
      end
    ...
    def plugin_select_init(options)
      ...
      if options[:select_test_map]
        options[:filter] = Minitest::Select::FileFilter.new(
          options[:select_test_map],
          options[:select_config],
          options[:select_changed_files],
        )
      elsif options[:select_min_length]
        options[:filter] = Minitest::Select::MinLengthFilter.new(options[:select_min_length])
      end
    end
  end
end
